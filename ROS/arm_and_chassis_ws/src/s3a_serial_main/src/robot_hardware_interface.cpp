	...
	motors_en(true)
{
	...
	readJointNames();
	//TODO Check N_MOTORS == n_joints

	curr = State(n_joints);
	prev = State(n_joints);
	tmp_pos.resize(n_joints);
	...
	ctrl = new Motor_Ctrl_Serial_Master();
...
void RobotHardwareInterface::read(ros::Duration elapsed_time) {
	...
	if(
		true && //TODO Hack until get_pos() work.
		all_motors_sim){
		for(int id = 0; id < n_joints; id++){
			// rad = % * s * rad/s
		...
		ctrl->get_pos(tmp_pos);
		for(int id = 0; id < n_joints; id++){
			curr.pos_fb[id] = sym_clamp(int(tmp_pos[id]), 90) * M_PI/180;
		}
	}
...
void RobotHardwareInterface::write(ros::Duration elapsed_time) {
	if(!all_motors_sim && motors_en){
		...
		for(int id = 0; id < n_joints; id++){
			tmp_pos[id] = sym_clamp(int(curr.pos_cmd[id] * 180/M_PI), 90);
		}

		ctrl->set_pos(tmp_pos);
	}
}
