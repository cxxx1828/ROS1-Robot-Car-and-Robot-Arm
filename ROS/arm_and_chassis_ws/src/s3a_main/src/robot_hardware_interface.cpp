...
// TODO Includes for working with driver file.
	...
	motors_en(true)
{
	...
	if(!all_motors_sim){
		// TODO Open driver.
		// Use drv_fd as file descriptor.
		// Log with ROS_WARN if cannot open driver.
	}
...
void RobotHardwareInterface::read(ros::Duration elapsed_time) {
	...
	if(all_motors_sim){
		...
		// TODO Read from driver to curr.pos_fb array.
		// From driver are read permilles.
		// It need to be converted to angles in rad with duty2rad().
	}
...
void RobotHardwareInterface::write(ros::Duration elapsed_time) {
	if(!all_motors_sim && motors_en){
		//TODO Write to driver from curr.pos_cmd array.
		// Need to do conversion from rad to permilles with rad2duty().
	}
}
